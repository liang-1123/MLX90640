...
/*  该函数从具有给定从地址的MLX90640设备读取所有必要的EEPROM数据到用户定义的MCU存储器位置

（Dump 转储）。为保证正常运行，分配的内存至少应为832个字。如果结果是-1，在通信期间发生了NACK，
...
/*该函数从具有给定从地址的MLX90640设备读取所有必要的帧数据到用户定义的MCU存储器位置。

为保证正常运行，分配的内存至少应为834个字。如果结果为-1，说明在通信过程中发生了NACK，

这不是一个有效的帧数据，否则如果结果为-8，说明数据在一定时间内无法获取，

帧数据损坏-最有可能的原因是I2C频率太低，否则结果是获取数据的子页面*/
...
/*该函数从给定的EEPROM数据数组中提取参数，并将值存储为MLX90640_API.h中定义的类型。

提取（Extract）参数（Parameters）后，不再需要EEPROM数据，其存储的内存可以重复使用。

如果返回值为-7，则指定位置的EEPROM数据不是有效的MLX90640 EEPROM，并且参数提取被中止。


`uint16_t *eeData — 指向数组的指针，该数组包含要从中提取参数的EEPROM

`*paramsMLX90640 *mlx90640 *— 指向类型为paramsMLX90640的变量的指针，在该变量中存储提取的参数。

注意，如果在线上有多个MLX90640设备，则可以使用paramsMLX90640类型的数组*/
...
//------------------------------------------------------------------------------
/* 该函数在适当的寄存器中写入所需的分辨率值(0x00到0x03)，

以便更改具有给定从地址的MLX90640设备的当前分辨率。

注意，上电复位后，分辨率将恢复到存储在EEPROM中的分辨率。

如果写入成功，返回值为0，如果通信过程中发生NACK，返回值为-1，如果写入值与预期值不相同，返回值为-2。

uint8_t resolution — MLX90640的当前分辨率
...
//------------------------------------------------------------------------------
/*该函数返回具有给定从属地址的MLX90640设备的当前分辨率。

请注意，当前分辨率可能与该设备的EEPROM中设置的分辨率不同。

如果结果为-1，说明在通信过程中发生了NACK，这不是一个有效的解析数据。*/
...
//------------------------------------------------------------------------------
/*该函数根据从MLX90640设备读取的帧数据-frame data、为该特定设备提取的参数-parameters和

用户定义的发射率-emissivity 和反射温度-reflected Temperature，计算帧中所有768个像素的对象温度。

为保证正常运行，分配的内存至少应为768个字

`uint16_t *frameData — 指向用户想要存储帧数据的MCU内存位置的指针

`paramsMLX90640 *params— 指向MCU内存位置的指针，其中存储了MLX90640设备已提取的参数

`float emissivity— 发射率由用户定义。发射率是被测物体的一种特性

`float tr— 反射温度由用户定义。如果物体的发射率小于1，则可能有一些温度从物体反射。为了补偿这一点，用户应该输入这个反射温度。可以使用传感器环境温度，但可能需要根据外壳进行一些转换。对于露天的MLX90640，温度变化为-8°C。

`float *result— 指向用户希望存储对象温度数据的MCU内存位置的指针
